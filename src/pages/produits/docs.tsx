...
export default function DocsPage() {
  ...
  const [currentTestimonial, setCurrentTestimonial] = useState(localized.testimonials?.[0] || null)
  
  useEffect(() => {
    if (localized.testimonials?.length) {
      const randomIdx = Math.floor(Math.random() * localized.testimonials.length)
      setCurrentTestimonial(localized.testimonials[randomIdx])
    }
  ...
  return (
    ...
    >
      ...
      <FadeInSection>
        <Testimonials testimonial={currentTestimonial} />
      </FadeInSection>
      <FadeInSection>
