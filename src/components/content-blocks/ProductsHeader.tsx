...
type HeaderLink = { title: string; url: string; type?: string; variant?: string }

const LoginIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
    <path d="M9 18V16H16V2H9V0H16C16.55 0 17.0208 0.195833 17.4125 0.5875C17.8042 0.979167 18 1.45 18 2V16C18 16.55 17.8042 17.0208 17.4125 17.4125C17.0208 17.8042 16.55 18 16 18H9ZM7 14L5.625 12.55L8.175 10H0V8H8.175L5.625 5.45L7 4L12 9L7 14Z" fill="currentColor"/>
  </svg>
)

const HeaderRight: React.FC<{ links?: HeaderLink[] }> = ({ links }) => {
  const loginLink = Array.isArray(links) && links.length > 0 ? links[links.length - 1] : null
  ...
  return (
    ...
    <div className="flex items-center gap-2 md:top-0 md:right-0 hidden lg:flex z-10 ml-auto">
      {Array.isArray(links) && links.map((link, idx) => {
        ...
        return (
          <>
            <Button key={`${link.title}-${idx}`} target="_blank" href={link.url} variant={variant} size="small">
              {link.title}
            </Button>
