...
export const RoadMap: React.FC<RoadMapData> = ({ title, content, items, link }) => {
      ...
      <div className="max-w-container mx-auto w-[100%]">
          ...
          {items.map((item, idx) => {
            ...
            if (isLast) {
              return (
                ...
                >
                  <span className="text-gray-850 font-bold text-base md:text-2xl z-10 relative text-center md:text-left">{item.alt}</span>
                  <div 
        ...
        {link && link.url && link.title && (
          <div className="flex justify-center mt-12 md:mt-10">
            <Button href={link.url} variant="tertiary_brand" target="_blank" icon={<ExternalLinkIcon />} iconPosition="left">
              {link.title}
            </Button>
          </div>
        )}
