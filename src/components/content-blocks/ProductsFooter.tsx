...
export const ProductsFooter: React.FC<{
  ...
  return productContent.footer ? (
    <div className="py-12 md:py-[120px] mx-auto bg-gray-025 text-center px-6 md:px-0">
      {logoProduct ? (
        <Image
          className="mx-auto w-[272px] md:max-w-[432px]"
          src={logoProduct.src}
          width={432}
          height={96}
          alt={`logo ${slug}`}
      ...
      <p
        className="pb-10 pt-2 text-gray-550 text-sm md:text-xl font-normal"
        dangerouslySetInnerHTML={{
          __html: productContent.footer?.text,
        }}
      />

      {productContent.footer?.links &&
        Array.isArray(productContent.footer.links) &&
        productContent.footer.links.length > 0 && (
